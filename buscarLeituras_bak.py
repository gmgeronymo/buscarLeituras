# Script para acessar banco de dados MDB
# Autor: Gean Marcos Geronymo
# Data: 23/01/2017

# utiliza o módulo pyodbc
...
import pyodbc    # módulo para abrir banco de dados MS ACCESS (MDB)
import psycopg2  # módulo para abrir banco de dados PostgreSQL (Condições Ambientais)
...
from PyQt5.QtWidgets import (QMainWindow, QWidget, QTableWidget,QTableWidgetItem,
                             QVBoxLayout, QHBoxLayout, QApplication, QCheckBox, QColorDialog, QDialog,
                            QErrorMessage, QFileDialog, QFontDialog, QFrame, QGridLayout, QGroupBox, 
                            QInputDialog, QLabel, QLineEdit, QMessageBox, QPushButton, QRadioButton, QComboBox)
...
class App(QWidget):
    ...
    def __init__(self):
        ...
        self.setWindowTitle("Leituras")
        self.resize(900, 600)

        self.caminho = caminhoTensao  # valor default
    ...
    def createOptionsGroupBox(self):
        ...
        # Banco de dados
        self.openBancoDadosLabel = QLabel()
        self.openBancoDadosLabel.setFixedWidth(300)
        self.openBancoDadosLabel.setText(self.caminho)        
        self.openBancoDadosLabel.setFrameStyle(frameStyle)
        self.openBancoDadosButton = QPushButton("Selecionar Banco de Dados")

        # LTR
        self.openRegistroLabel = QLabel()
        self.openRegistroLabel.setFixedWidth(300)
        self.openRegistroLabel.setFrameStyle(frameStyle)
        ...
        optionsGroupBoxLayout.setColumnStretch(1, 1)
        optionsGroupBoxLayout.setColumnMinimumWidth(1, 100)

        optionsGroupBoxLayout.addWidget(self.tensao, 0, 0)
        optionsGroupBoxLayout.addWidget(self.openBancoDadosButton, 1, 0)
        optionsGroupBoxLayout.addWidget(self.openRegistroButton, 2, 0)

        optionsGroupBoxLayout.addWidget(self.corrente, 0, 1)
        optionsGroupBoxLayout.addWidget(self.openBancoDadosLabel, 1, 1)      
        optionsGroupBoxLayout.addWidget(self.openRegistroLabel, 2, 1)
    ...
    def createAcoesGroupBox(self):
        ...
        self.plotarTempUmid = self.createButton("Plotar Temperatura e Umidade", self.plotTempUmid)

        acoesGroupBoxLayout = QHBoxLayout()
        acoesGroupBoxLayout.addWidget(self.buscarLeituras)
        acoesGroupBoxLayout.addWidget(self.plotarTempUmid)
    ...
    def createExportarGroupBox(self):
        ...
        #exportarGroupBoxLayout.addStretch()
    ...
    def buscarLeituras(self):
        ...
        try:
            ...
            self.dataValue.setText(self.resultados.data[sorted(self.resultados.diferencas.keys())[0]])
            ...
            for coluna in range(colunasTabela):
                for linha in range(linhasTabela):
                    if coluna == 0: # primeira coluna: labels
                        ...
                        # cor de fundo
                        self.tableWidget.item(linha, 0).setBackground(QColor(200,200,200))
                        ...
                        freq = sorted(self.resultados.diferencas.keys())
                        if freq[coluna-1] < 1:
                            freq_str = str(freq[coluna-1])
                        else:
                            freq_str = str(int(freq[coluna-1]))

                        if linha == 0:  # cabeçalho com as frequencias
                            self.tableWidget.setItem(linha,coluna, QTableWidgetItem(freq_str))  
                            self.tableWidget.item(linha,coluna).setBackground(QColor(200,200,200))
                            ...
                            self.tableWidget.setItem(linha,coluna, QTableWidgetItem("{0:.2f}".format(np.mean(self.resultados.diferencas[freq[coluna-1]]))))
                            self.tableWidget.item(linha, coluna).setFont(font_bold)
                            # bloquear celulas
                            self.tableWidget.item(linha, coluna).setFlags(Qt.ItemIsEnabled)
                        elif linha == self.resultados.linhas + 3:    # desvio padrão
                            self.tableWidget.setItem(linha,coluna, QTableWidgetItem("{0:.2f}".format(np.std(self.resultados.diferencas[freq[coluna-1]], ddof=1))))
                            self.tableWidget.item(linha, coluna).setFont(font_bold)
                            # bloquear celulas
                            self.tableWidget.item(linha, coluna).setFlags(Qt.ItemIsEnabled)
    ...
    def copiarDiferencas(self):
        try:
            ...
            for i in range(self.resultados.linhas):
                j = 0
                for freq in sorted(self.resultados.diferencas):
    ...
    def plotTempUmid(self):
        try:
            self.resultados.plotCondicoesAmbientais()
        except:
            QMessageBox.critical(self, "Erro",
                    ...
                    for coluna in range(29):
                        if linha == 0:
                            ...
                            if coluna in range(5,28):
                                try:
                                    clipboard += str(self.resultados.diferencas[freq[coluna-5]][linha-3][0]).replace('.',',') + '\t'
                                except:
                                    clipboard += '\t'
                            elif coluna == 28:
                                try:
...
class Resultados(object):
    ...
    def __init__(self, caminhoBancoDados, nomeRegistro):
        ...
        conn = pyodbc.connect(r'DRIVER={Microsoft Access Driver (*.mdb, *.accdb)}; DBQ='+self.caminhoBancoDados+';PWD='+self.passwd)
        cur = conn.cursor()
    ...
    def getDiferencas(self, valprog, faixa792):
        ...
        for row in self.registro_rows:
            ...
            self.diferencas[newFreq] = cur.execute("SELECT DIFERENCA FROM Leituras WHERE CODREG="+str(self.id.CODREG)+" AND CODPONTO ="+str(row.CODPONTO)).fetchall()
            self.data[newFreq] = row.DATACAL.strftime("%d/%m/%y")
            self.hora[newFreq] = row.HORACAL.strftime("%H:%M")
    ...
    def getCondicoesAmbientais(self):
        ...
        self.temperaturaMedia = "{0:.1f}".format(np.mean(self.temperature)).replace('.',',')
        self.umidadeMedia = "{0:.1f}".format(np.mean(self.humidity)).replace('.',',')
        
        cur.close()
        conn.close()
        return

    def plotCondicoesAmbientais(self):
        fig, ax1 = plt.subplots()
        lns1 = ax1.plot(self.date, self.temperature, 'r-', label='Temperatura')
        yres = 0.1 # resolucao do eixo de temperatura
        start, stop = ax1.get_ylim()
        ticks = np.arange(start, stop + yres, yres)
        ax1.set_yticks(ticks)
        myFmt = mdates.DateFormatter('%d/%m - %H:%M')
        ax1.xaxis.set_major_formatter(myFmt)
        ax1.format_xdata = mdates.DateFormatter('%d/%m - %H:%M')
        ax1.grid(True)
        ax1.set_xlabel('Dia / Hora')
        ax1.set_ylabel('Temperatura [ºC]')
        ax1.yaxis.set_major_formatter(mtick.FormatStrFormatter('%.1f'))
        ax2 = ax1.twinx()
        lns2 = ax2.plot(self.date, self.humidity, 'b-', label='Umidade')
        ax2.set_ylabel('Umidade [% u.r.]')
        lns = lns1+lns2
